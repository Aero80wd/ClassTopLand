...
void TableEditWidget::saveSyncSettings() {
    ...
    connect(reply,&QNetworkReply::finished,this,[=]{
        if (reply->error()){
            ...
            m_joConfig["sync_open"] = ui->syncopen->isChecked();
            m_joConfig["sync_server"] = ui->sync_server->text();
            m_joConfig["sync_server_port"] = ui->sync_port->value();
            m_joConfig["class_name"] = ui->sync_class->text();
            QMessageBox::information(this,tr("提示"),tr("重启生效"));
        }
...
void TableEditWidget::setConfig(QJsonObject obj){
    ...
    ui->syncopen->setChecked(m_joConfig.value("sync_open").toBool());
    ui->sync_server->setText(m_joConfig["sync_server"].toString());
    ui->sync_port->setValue(m_joConfig["sync_server_port"].toInt());
    ui->sync_class->setText(m_joConfig["class_name"].toString());
